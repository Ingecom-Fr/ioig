...
{
public: 
    HC_SR04() :trigPin(TRIG_PIN, Output),
               echoPin(ECHO_PIN, Input){}
...
private: 
       ioig::Gpio trigPin;
       ioig::Gpio echoPin;
       static constexpr unsigned TRIG_PIN=11;
       static constexpr unsigned ECHO_PIN=10;
...
int main() 
{
    ...
    puts("HC-SR04 + LiquidCrystal_I2C");   
	...
	lcd.setCursor(0,0);
    lcd.print(ingeStr);
    std::stringstream lcdInfo;
    int x=0;

	while (1) 
	{
        lcdInfo << "Distance(cm):" << hcsr04.getDistanceCm();

   	    lcd.setCursor(x,0);
        x = x + ingeStrlen >= 20 ? 0 : x + 1;
        lcd.print(ingeStr);
        
        lcd.setCursor(0,1);
        lcd.print(lcdInfo.str().c_str());
