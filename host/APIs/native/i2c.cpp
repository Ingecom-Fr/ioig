...
//object private data
static ioig::Packet txBufffer;
static ioig::Packet rxBufffer;
...
uint8_t I2C::endTransmission(bool stopBit)
{
    ...
    int ret = 4;
    if (txBufffer.getPayloadLength()>0) 
    {
        ret = write(_addr, txBufffer.getPayloadBuffer() , txBufffer.getPayloadLength() , !stopBit);	
	}else 
    {	
        // we are scanning, return 0 if the addresed device responds with an ACK
		uint8_t buf[1];
		ret = read(_addr, buf, 1, !stopBit);		        
    }
    return ret;    
...
size_t I2C::requestFrom(uint8_t address, size_t len, bool stopBit)
{
    checkAndInitialize();

    rxBufffer.reset();

    if (len > rxBufffer.getFreePayloadSlots()) 
    {            
        LOG_ERR(TAG, "Read buffer overflow, requested %d bytes, available %d bytes", (int)len, rxBufffer.getFreePayloadSlots());    
        return 0;
    }

	int ret = read(address, rxBufffer.getPayloadBuffer(), len, !stopBit);
    if (ret != 0) 
    {
        return 0;
    }
...
size_t I2C::requestFrom(uint8_t address, size_t len)
{
    checkAndInitialize();
...
size_t I2C::write(uint8_t data)
{
    ...
    if (txBufffer.addPayloadItem8(data) < 0) 
    {
...
size_t I2C::write(const uint8_t* data, int len)
{
    checkAndInitialize();
    
    if (len > (int)txBufffer.getFreePayloadSlots()) 
    {          
        LOG_ERR(TAG, "Write buffer overflow, requested %d bytes, available %d bytes", len, txBufffer.getFreePayloadSlots());
        len = txBufffer.getFreePayloadSlots();
    }

    txBufffer.addPayloadBuffer(data, len);
...
int I2C::available()
{
    return rxBufffer.getFreePayloadSlots();
}
